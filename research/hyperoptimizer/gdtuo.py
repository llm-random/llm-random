...
class Optimizable:
    ...
    def clip_grad_norm(self, max_norm, norm_type=2):
        """Clip the gradient norm of all parameters to a maximum value."""
        torch.nn.utils.clip_grad_norm_(
            self.all_params_with_gradients, max_norm, norm_type
        )
        self.optimizer.clip_grad_norm(max_norm, norm_type)
...
class SGD(Optimizable):
    ...
    def __init__(self, alpha=0.01, mu=0.0, optimizer=NoOpOptimizer()):
        self.mu = mu
        self.state = {}
        parameters = {"alpha": torch.tensor(alpha), "mu": torch.tensor(mu)}
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            if name not in self.cache:
                self.cache[name] = {"G": torch.zeros_like(param) + 1e-1}
            g = param.grad.detach()
            self.cache[name]["G"] = G = self.cache[name]["G"].detach() + torch.square(g)
            params[name] = (
                param.detach()
                - self.parameters["alpha"] * g / torch.sqrt(G + self.eps).detach()
            )
...
class RMSProp(Optimizable):
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            if name not in self.cache:
                self.cache[name] = {"s": torch.zeros_like(param)}
            g = param.grad.detach()
            self.cache[name]["s"] = s = gamma * self.cache[name]["s"].detach() + (
                1.0 - gamma
            ) * torch.square(g)
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            if name not in self.cache:
                self.cache[name] = {"s": torch.zeros_like(param)}
            g = param.grad.detach()
            self.cache[name]["s"] = s = self.gamma * self.cache[name]["s"].detach() + (
                1.0 - self.gamma
            ) * torch.square(g)
            self.all_params_with_gradients.append(s)
            params[name] = param.detach() - alpha * g / torch.sqrt(s + self.eps)
        ...
        for name, param in params.items():
            if name not in self.cache:
                self.cache[name] = {
                    ...
                    "v": torch.zeros_like(param) + self.eps
    ...
    def step(self, params):
        ...
        for name, param in params.items():
            if name not in self.cache:
                self.cache[name] = {
                    "m": torch.zeros_like(param),
                    "v": torch.zeros_like(param) + 10.0**self.log_eps
                    # NOTE that we add a little `fudge factor' here because sqrt is not
                    # differentiable at exactly zero
                }
