...
class TestBatchedFeedForward(GeneralTestCase):
    def test_basic(self):
        ...
        layer = research.inverted.moe_layers.ffs.BatchSplitFF(
            register_list=[],
            dm=dm,
            dff=dff,
            expertsets=sets,
...
class FactoredDenseTest(GeneralTestCase):
    def test_basic(self):
        ...
        layer = research.inverted.moe_layers.ffs.FactoredDense(dinp, dout, modules=4)
    ...
    def test_more_dims(self):
        ...
        layer = research.inverted.moe_layers.ffs.FactoredDense(dinp, dout, modules=2)
...
class TestGeneralizedReLU(GeneralTestCase):
    def test_basic(self):
        batch, dinp = 4, 32
        bias = False
        layer = research.inverted.moe_layers.ffs.GeneralizedReLU(dinp, bias)
        input = torch.normal(0.0, 1.0, (batch, dinp))
        output = layer(input)
...
class BERTSparseTest(GeneralTestCase):
    ...
    def test_basic(self):
        ...
        factored_dense_fun = lambda: research.inverted.moe_layers.ffs.FactoredDense(
            dm, dm, modules
        )
        ...
        encoder_tower = llm.TransformerTower(
            ...
            dm,
            (
                lambda: research.inverted.moe_layers.ffs.BatchSplitFF(
                    [], dm, dff, 4, 4, 4
                )
            ),
