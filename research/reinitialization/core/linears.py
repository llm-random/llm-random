...
@ash.check("... inp -> ... out")
class PruneLinear(lizrd.core.modules.Linear):
...
class MagnitudePruneLinear(lizrd.core.modules.Linear):
...
class SeparateDirectionMagnitudeFF(nn.Module):
    def __init__(
        ...
        self.dir1 = lizrd.core.modules.Linear(dmodel, dff, bias=bias_first)
        self.magnitude = nn.parameter.Parameter(
            torch.ones(dff), requires_grad=magnitude_requires_grad
        )
        self.dir2 = lizrd.core.modules.Linear(dff, dmodel, bias=bias_second)
