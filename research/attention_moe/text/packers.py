...
from .tokenizers import AbstractTokenizer, BertTokenizer
...
class GPTPacker(
    ...
    def get_sample(self) -> LLMExample:
        ...
        while True:
            document, document_id = self.dataset.get_document()
            tokens = self.tokenizer.text_to_ids(document)
            documents_ids.extend([document_id] * len(tokens) + [-1])
        ...
        documents_ids = list(take_circular(documents_ids, sample_start, sample_end))
