...
from research.context_scaling.attention import VanillaAttention
...
def get_attention_layer(args):
    ...
    if args.attention_mode == "vanilla":
        attention_layer_fun = lambda: VanillaAttention(
            ...
            use_rope=(args.positional_encoding == "rope"),
            seq_len=args.cutoff,
...
def get_norm_class(norm_class, norm_eps):
    if norm_class == "layer_norm":
        return partial(torch.nn.LayerNorm, eps=norm_eps)
    elif norm_class == "rms_norm":
        return partial(torch.nn.RMSNorm, eps=norm_eps)
    else:
        raise NotImplementedError(f"Norm type {norm_class} not implemented")


def get_residual_layer(args):
    norm_class = partial(get_norm_class(args.norm_class), args.dmodel)
