...
from research.token_reduction.layer_manager import LayerManager
...
def keep_given_indeces_old(input, dim, index):
    """
...
def keep_given_indeces(input, index):
...
class Trainer:
    ...
    def train(self, n_steps: int):
        ...
        self._initialize_fsdp_model(self.init_cutoff)
    ...
    def _initialize_fsdp_model(self, init_cutoff):
        if isinstance(self.model, FSDP):
            ...
            with torch.no_grad():
                _ = self.model(
                    torch.zeros((self.batch_size, init_cutoff), dtype=torch.int)
                )
