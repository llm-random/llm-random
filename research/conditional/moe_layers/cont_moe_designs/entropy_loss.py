...
class ContinuousMoEEntropyLoss(ContinuousMoeBaseClass):
    ...
    def get_entropy(self, merge_weights, emit_weights):
        entropy_merge = entropy(merge_weights).mean() / torch.log(torch.Tensor([self.group_size]))
        entropy_emit = entropy(emit_weights).mean() / torch.log(torch.Tensor([self.group_size]))
