def check_args(args):
    ...
    print(f"PROJECTED DISTILLATOPM {args.projected_distillation}")

    if args.projected_distillation:
        assert not ((args.projected_dmodel is None) and (args.projected_dff is None))
        if args.projected_dmodel * 4 != args.projected_dff:
            print(f"WARNING - not standard hidden dmodel/dff ratio: {args.projected_dmodel}/{args.projected_dff}")
    else:
        assert (args.projected_dmodel is None) and (args.projected_dff is None)
