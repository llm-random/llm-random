...
from research.mole.moe_layers.load_balancing_loss import calculate_load_balancing_loss, calculate_biased_balancing_loss, calculate_z_loss
...
class RouterBiasTargetSetterManager:
    def __init__(self, model: torch.nn.Module, router_target_bias: torch.Tensor):
        ...
        if len(self._layers) == 0:
            raise Exception("No InputWiseRouterBias modules in model")
    ...
    def calculate_gate(self, x, batch_size, seq_len):
        with measure_time(self, "expert_embedding"):
            if self.use_torch_bmm:
                gate = self.get_gate().unsqueeze(0).expand(batch_size, -1, -1) # dev TODO why?
                # print("gate_out = self.calculate_gate(x, batch_size, seq_len).T ------------------------------------------------------")#dev
                # print("gate_logits = torch.bmm(x, gate).permute(2, 0, 1)")
                # print(f"x.shape {x.shape}")#dev
                # print(f"gate.shape {gate.shape}")#dev
...
class TokenGatingBiased(MoeGating, InputWiseRouterBias):
    def __init__(
        ...
        **kwargs,
        ...
        self.biased_balancing_loss_fn = torch.nn.BCEWithLogitsLoss() #dev
