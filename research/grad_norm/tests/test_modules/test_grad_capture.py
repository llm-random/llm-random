...
def test_grad_capture_logs():
    ...
    assert logs["activation_norms_mean"] == torch.mean(torch.norm(x, dim=-1))
    assert logs["activation_norms_std"] == torch.std(torch.norm(x, dim=-1))
    assert logs["grad_norms_mean"] == torch.mean(torch.norm(grad, dim=-1))
    assert logs["grad_norms_std"] == torch.std(torch.norm(grad, dim=-1))
    assert logs["activation_norms_mean"] == torch.mean(torch.norm(x, dim=-1))
    assert logs["activation_norms_std"] == torch.std(torch.norm(x, dim=-1))
