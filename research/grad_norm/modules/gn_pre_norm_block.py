...
def GradMofiedPreNormBlock(
    ...
    on_residual_blocks = [
        ("residual", Residual(nn.Sequential(OrderedDict([b for b in inside_residual_blocks if b is not None])))),
        ("post_add_gn", gn_layer()) if gn_placement.post_add else None
    ]

    return nn.Sequential(
        OrderedDict([b for b in on_residual_blocks if b is not None])
    )
