...
def make_loss_and_gradient_function(
    ...
    if loss_checkpoint_chungs == 0:
        ...
        raise NotImplementedError(
            "Loss chungization not supported"
        )
...
def run_backward(
    ...
    with torch.autocast(device_type="cuda", enabled=False, dtype=mixed_precision_dtype):  # dev do i need to have configured pe and att masks here?
        if scaler is None:
            loss.backward()
        else:
