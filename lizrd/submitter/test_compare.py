import os
import unittest
from lizrd.submitter.prepare_hostname import HostInfraConfig
from unittest.mock import MagicMock
from constants import MachineBackend


class TestClusterConfigs(unittest.TestCase):
    def test_ideas_cluster_config(self):
        HostInfraConfig.get_machine_backend = MagicMock(
            return_value=MachineBackend.IDEAS
        )
        host = HostInfraConfig(
            interactive_session=True,
            job_repository_path=os.getcwd(),
            n_gpus=1,
            cpus_per_gpu=10,
            time="1:00:00",
            job_name="super_random_name",
        ...
        returned_args = host.get_setup_arguments()
