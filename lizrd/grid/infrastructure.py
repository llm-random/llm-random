...
class IdeasBackend(MachineBackend):
    ...
    def get_subprocess_args(
        ...
        assert (
            setup_args["num_consecutive"] == 1
        ), "Only one consecutive job is supported on Ideas backend"
        return [
            slurm_command,
...
class EntropyBackend(MachineBackend):
    ...
    def get_subprocess_args(
        ...
        assert (
            setup_args["num_consecutive"] == 1
        ), "Only one consecutive job is supported on Entropy backend"
        return [
            slurm_command,
            "--partition=a100",
...
class AWS1Backend(MachineBackend):
    ...
    def get_subprocess_args(
        ...
        assert (
            setup_args["num_consecutive"] == 1
        ), "Only one consecutive job is supported on AWS backend"
        return [
            "singularity",
            "run",
            *singularity_env_arguments,
