...
class TrainRun:
    def __init__(
        ...
        **_,
        ...
        self.finished = (
            sys_state == "Inactive"
            and np.isfinite(self.loss)
            and step == self.n_steps == args_final_lr_step
            and all([getattr(self, k) == v for k, v in fixed.items()])
            and (
                self.n_params > 20000000
                or self.n_steps > 20000
...
def calculate_n_params_from_flops(
    flops, n_steps, expansion_rate, granularity, **params
):
...
def calculate_n_params_and_steps_from_flops(
    flops, expansion_rate, granularity, scaling_laws, **params
):
    ...
    res_raw = minimize_scalar(
        fun2
    )  # , options=dict(disp=1))   , method='bounded', bounds=(1, 1e6))
