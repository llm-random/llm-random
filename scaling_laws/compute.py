...
def plot_params(
    ...
    **_,
    ...
    for ii, scaling_law in enumerate(scaling_laws):
        model_sizes = {
            k: (v, np.inf, dict(flops=np.inf)) for k, v in show_model_sizes.items()
        }
        ...
        for (group, indices), color in zip(groups, colors):
            ...
            results = [
                scaling_law.resolve_params(**group_dict, **{axis_dim: np.power(10, a)})
                for a in A_values
            ]
        ...
        for B_p, b_opt, color, name, minimal in zip(
            B_predictions, B_opt_params, colors, names, is_min
        ):
            if full_flops:
                for i, (pred, params) in enumerate(zip(B_p, b_opt)):
                    ...
                    for k, (size, perplexity, old_params) in model_sizes.items():
                        if (
                            pred < perplexity
                            and params["n_params"] >= float(size)
                            and old_params["flops"] > params["flops"]
                        ):
