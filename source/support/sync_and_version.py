...
from source.support.code_versioning import find_git_root, version_code
...
def cd_to_root_dir():
    ...
    assert repo.remotes.origin.url in [
        "git@github.com:SECRET_REPO/SECRET_REPO.git",
    ], "You're not in the right repo! Move to the SECRET_DIRECTORY folder, and make sure your origin is the SECRET_REPO repo. Aborting..."
    os.chdir(git_root)
...
def SECRET_CLUSTER2_user_to_workdir(connection):
    d = {}
...
def get_base_directory(connection):
    if connection.host == "SECRET_HOST2":
        base_dir = f"SECRET_PATH1/{SECRET_CLUSTER2_user_to_workdir(connection)}/SECRET_DIRECTORY"
    else:
        base_dir = f"~/SECRET_DIRECTORY"
    return base_dir


def get_proxy_command(connection):
    if connection.host == "SECRET_HOST1":
...
def set_up_permissions(host):
    try:
        with ConnectWithPassphrase(host) as connection:
            path = f"{get_base_directory(connection)}/source/scripts/grid_entrypoint_SECRET_CLUSTER2.sh"
...
if __name__ == "__main__":
    ...
    base_dir = rsync_to_remote(args.host, working_dir + "/source")
