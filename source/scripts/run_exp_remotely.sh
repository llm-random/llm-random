#!/bin/bash
# INSTRUCTIONS:
# 1. needs to be called from somewhere in main directory
# 2. needs to be called with the host as the first argument (as configured in ~/.ssh/config, e.g. gpu_SECRET_CLUSTER1)
# 3. needs to be called with the path to config file as the second argument (e.g. "runs/my_config.yaml")
# EXAMPLE USAGE: bash source/scripts/run_exp_remotely.sh SECRET_CLUSTER2 source/configs/quick.json
set -e
