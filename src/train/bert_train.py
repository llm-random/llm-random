...
from src.support import profile
...
def train_step(model, optimizer, pdataset, step=0):
    ...
    assert isinstance(processed_batch, src.datasets.processed_batch.ProcessedBatch)
...
def eval_step(model, pdataset, step=0, sample=10):
    ...
    with torch.no_grad():
        ...
        for sample_i in range(sample):
            processed_batch = pdataset.get_batch(BATCH_SIZE)
            assert isinstance(
                processed_batch, src.datasets.processed_batch.ProcessedBatch
            )
            x_set = processed_batch.masked_tokens
            y_class_set = processed_batch.swapped
            y_token_set = processed_batch.tokens
...
if __name__ == "__main__":
    ...
    if USE_CLEARML:
        task = Task.init(
            project_name="jaszczur/sparsity/tests", task_name=f"{nametext} {timestamp}"
        )
        TASK = task
    WRITER = writer
