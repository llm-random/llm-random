...
from src.datasets import wikibookdata
from src.datasets.processor import BERTSentenceProcessor, GPTSentenceProcessor
import src.datasets.processor

from src.datasets.processed_batch import (
    ProcessedBERTBatch,
    ProcessedBatch,
    ProcessedGPTBatch,
)
from src.datasets.c4 import C4Dataset
import src.datasets.processor
...
def get_processed_dataset(
    ...
    if dataset_type == "wikibook":
        ...
        if model_type == "bert":
            processor = src.datasets.processor.BERTSentenceProcessor(
                max_total_length=max_total_length,
                mask_percent=mask_percent,
            )
        elif model_type == "gpt":
            processor = src.datasets.processor.GPTSentenceProcessor(
                max_total_length=max_total_length,
            )
