...
# EXAMPLE USAGE: bash src/scripts/run_exp_remotely.sh atena src/configs/quick.json
set -e

source venv/bin/activate
# run your python script
python3 -m src.support.sync_and_version --host $1
...
run_grid_remotely() {
  ...
  script+="; tmux send-keys -t $session_name 'python3 -m src.scripts.grid --config_path=$config --git_branch=$git_branch"
